...
app.get('/purchaseitems/:items/:user_id/:user_password', function (req, res) {
  // remove ' ' chars
  req.params.items.replace("'",'');
  // turn into array
  const items_array = req.params.items.split(',');
  purchaseItems(items_array, req.params.user_id, req.params.user_password);
  res.send('nice!');
})
